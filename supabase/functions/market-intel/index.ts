...
function quoteSignal(q: Quote): Signal {
  ...
  // (Left intact per scope — do NOT alter the %-move fallback in this pass.)
  if (!q.eng) {
...
// FX / metals / energy market-week helpers live in market-week.ts (unit-tested).
// They are imported at the top of this file: isForexMarketClosed / nextForexOpen /
// nextForexClose / fxWhen / FX_OPEN_DOW / FX_OPEN_HOUR_UTC / FX_CLOSE_DOW / FX_CLOSE_HOUR_UTC.
...
// Forex session label for the Telegram signal MESSAGE lives in session-label.ts
// (sessionDisplayLabel) so it can be unit-tested independently. It mirrors the
// app's Confluence "Forex Sessions" panel and returns "Markets Quiet" when no
// session is open.
  ...
  try {
    ...
    if (force) {
      ...
      for (const name of wanted) {
        ...
        for (const tfDef of TIMEFRAME_CASCADE) {
          const activeFrom = now + tfDef.delayMs;
          const expiresAt = activeFrom + goldExpiryMs(symbol, tfDef.tf, entry, tp, sl);
