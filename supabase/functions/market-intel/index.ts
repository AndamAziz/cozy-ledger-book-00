...
// ───────────────────── channel growth: milestones + weekly stats ─────────────────────

// Live member count of the public channel via the Telegram gateway.
async function getMemberCount(): Promise<number | null> {
  const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
  const TELEGRAM_API_KEY = Deno.env.get("TELEGRAM_API_KEY");
  if (!LOVABLE_API_KEY || !TELEGRAM_API_KEY) return null;
  try {
    const res = await fetch(`${TELEGRAM_GATEWAY}/getChatMemberCount`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${LOVABLE_API_KEY}`,
        "X-Connection-Api-Key": TELEGRAM_API_KEY,
    ...
    const d = await res.json().catch(() => ({}));
    if (res.ok && d.ok && typeof d.result === "number") return d.result;
  } catch { /* ignore */ }
  return null;
}

// Subscriber milestones that trigger a celebration post.
const MILESTONES = [10, 50, 100, 250, 500, 1000];

function milestoneMessage(n: number): string {
  const special = n >= 100;
  return [
    "━━━━━━━━━━━━━━━",
    special ? "🎉🎊 <b>BIG MILESTONE!</b> 🎊🎉" : "🎉 <b>MILESTONE REACHED!</b>",
    "━━━━━━━━━━━━━━━",
    `🥳 We reached <b>${n} members</b>!`,
    `<i>گەیشتینە ${n} ئەندام!</i>`,
    "",
    "🙏 <b>شکرێکی زۆر بۆ پشتیوانیتان</b>",
    "<i>Thank you so much for your support!</i>",
    special ? "🚀 <b>Bigger signals, bigger wins ahead.</b>" : "",
    "",
    "📱 https://t.me/goldmarketai",
    "━━━━━━━━━━━━━━━",
  ].filter(Boolean).join("\n");
...
// Celebrate any newly-crossed milestone (only the highest new one, to avoid spam).
async function evaluateMilestones(): Promise<string[]> {
  const count = await getMemberCount();
  if (count == null) return [];
  const state = await getState("milestones");
  const reached: number[] = Array.isArray(state.reached) ? (state.reached as number[]) : [];
  const due = MILESTONES.filter((m) => count >= m && !reached.includes(m));
  if (!due.length) {
    await setState("milestones", { reached, lastCount: count });
    return [];
  }
  const merged = [...new Set([...reached, ...due])];
  await setState("milestones", { reached: merged, lastCount: count });
  return [milestoneMessage(Math.max(...due))];
}

// ───────────────────── weekly performance (Mondays) ─────────────────────
const WEEKLY_UTC_HOUR = 7; // ~08:00 BST Monday morning
...
// Weekly recap: gold win-rate, best signal, subscriber growth. Fires Monday ~08:00 BST.
async function evaluateWeeklyStats(opts?: { force?: boolean }): Promise<string | null> {
  const now = new Date();
  if (!opts?.force) {
    if (now.getUTCDay() !== 1) return null;             // Monday only
    if (now.getUTCHours() !== WEEKLY_UTC_HOUR) return null;
    const state = await getState("weekly_stats");
    if (state.lastWeek === isoWeekKey(now)) return null;
  }
  ...
  const goldWon = goldRows.filter((r) => r.status === "target_hit").length;
  const goldWinRate = goldRows.length ? Math.round((goldWon / goldRows.length) * 100) : 0;

  const allWon = rows.filter((r) => r.status === "target_hit").length;
  const allWinRate = rows.length ? Math.round((allWon / rows.length) * 100) : 0;
  const totalPips = rows.reduce((s, r) => s + rowPips(r), 0);

  const winners = rows.filter((r) => r.status === "target_hit").sort((a, b) => rowPips(b) - rowPips(a));
  const best = winners[0];

  // Subscriber growth vs last week's recorded count.
  const count = await getMemberCount();
  const wkState = await getState("weekly_stats");
  const prevSubs = typeof wkState.lastSubs === "number" ? (wkState.lastSubs as number) : null;
  let subsLine: string | null = null;
  if (count != null) {
    subsLine = prevSubs == null
      ? `👥 Subscribers: <b>${count}</b>`
      : `👥 Subscribers: <b>${prevSubs} → ${count}</b> ${count >= prevSubs ? "▲" : "▼"}`;
  }

  const dateLabel = now.toLocaleDateString("en-GB", {
    month: "short", day: "numeric", year: "numeric", timeZone: "Europe/London",
  });
  ...
  const lines: string[] = [
    ...
    `📅 ${esc(dateLabel)}`,
    "",
    `🥇 Gold Win Rate: <b>${goldWinRate}%</b> (${goldRows.length} ${goldRows.length === 1 ? "trade" : "trades"})`,
    `📈 Overall Win Rate: <b>${allWinRate}%</b>`,
    `🎯 Total Pips: <b>${pipsStr(totalPips)}</b>`,
  ];
  if (best) {
    const meta = ASSET_META[best.asset] ?? { emoji: "🥇", name: best.asset };
    lines.push(`🏆 Best Signal: <b>${pipsStr(rowPips(best))} pips</b> (${meta.emoji} ${meta.name})`);
  }
  if (subsLine) lines.push(subsLine);
  if (rows.length === 0) {
    lines.push("", "<i>هیچ سیگناڵێکی داخراو لەم هەفتەیەدا نەبوو</i>");
  }
  lines.push(
    "",
    "🙏 <b>سوپاس بۆ هاوبەشیتان · Thanks for trading with us!</b>",
    "📱 https://t.me/goldmarketai",
    "━━━━━━━━━━━━━━━",
    `<i>🕒 ${nowStamp()}</i>`,
  );
  ...
  if (!opts?.force) {
    await setState("weekly_stats", { lastWeek: isoWeekKey(now), lastSubs: count ?? prevSubs ?? null });
  }
  return lines.join("\n");
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, sent, targetsSent, sessionOpenAlerts: sessionOpenAlerts.length, specialAlerts: specialAlerts.length, dailySummary: dailySummary ? 1 : 0, weeklyStats: weeklyStats ? 1 : 0, milestoneAlerts: milestoneAlerts.length, signalAlerts: signalAlerts.length, outcomeAlerts: outcomeAlerts.length, calendarAlerts: calendarAlerts.length, newsAlerts: newsAlerts.length, quotes: lastQuotes.length }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
