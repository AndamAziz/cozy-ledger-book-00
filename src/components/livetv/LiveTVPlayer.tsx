...
import { containerFromExt, engineChain, isProgressiveContainer, type Engine } from '@/lib/containerSniff';
...
export function LiveTVPlayer({
  ...
  const engines = useMemo<Engine[]>(() => {
    ...
    // VOD leads with the native <video> element. Movies and series are often
    // served as .mp4, which mpegts.js cannot parse at all -- it only handles
    // MPEG-TS and FLV -- so the player sat on "Connecting to stream..." while
    // bytes arrived and were thrown away. The VPS proxy forwards Range, so
    // native playback also restores seeking. A .ts VOD falls through to
    // mpegts.js on the next rung exactly as before.
    if ((channel.kind ?? 'live') !== 'live') {
      const vod: Engine[] = ['native', 'mpegts'];
      if (Hls.isSupported()) vod.push('hls');
      return vod;
    }
  ...
  useEffect(() => {
    ...
    const armWatchdog = () => {
      ...
      // A multi-GB VOD whose moov atom sits at the tail makes Chrome seek to
      // the end of the file before it can decode a single frame; on this
      // provider that takes well past fifteen seconds, and tearing the
      // element down mid-seek restarted the whole ladder. Live still fails
      // fast -- a live feed that has sent nothing in 15 s really is dead.
      }, usingDirect ? 8_000 : (isLiveKind ? 15_000 : 45_000));
    };
    const done = () => {
      clearWatchdog();
      setLoading(false);
    ...
    const startTimer = window.setTimeout(() => {
      ...
      // Every kind, not just live. Movies and series were still being pumped
      // through the Edge Function, which is where the stalling came from: the
      // same wall clock ceiling and the same slow hop that live was moved off.
      // The VPS proxy forwards Range, so seeking keeps working for VOD.
      {
        ...
        void fetch(`${proxySrc}&handoff=1`)
          ...
          .then((j) => {
            ...
            if (typeof j?.url === 'string' && j.url.startsWith('https://')) {
              ...
              // direct-is-dead path, so a large VOD -- whose moov atom sits at
              // the end of a multi-GB file -- was torn down before its first
    ...
    <div data-tv-scope className="fixed inset-0 z-[90] flex flex-col bg-black/95 backdrop-blur-xl md:bg-[#07070b]/97 lg:flex-row lg:items-stretch">
      ...
      {(noAudio || codecIssue) && !loading && !error && (
        <div className="shrink-0 border-t border-white/10 bg-[#0b0b10]/90 px-4 py-3 sm:py-4">
          <div className="mx-auto flex max-w-3xl flex-col items-center justify-center gap-2.5 sm:flex-row sm:gap-4">
            <span className="text-center text-[11px] font-semibold leading-relaxed text-white/85 sm:text-left sm:text-xs">
              {codecIssue
