...
export function CategoryAccordion({ category, kind, poster, onPlay, onSeeAll }: Props) {
  ...
  // The index already ships the first channels of each category, so opening a
  // row is instant and costs the provider (single connection!) nothing.
  const seeded = (category.preview?.length ?? 0) >= Math.min(PREVIEW_LIMIT, category.count || PREVIEW_LIMIT);
  const { data, isLoading, isError, error, refetch } = useIptvChannels(category.id, open && !seeded, PREVIEW_LIMIT);
  const channels = data?.channels ?? (seeded ? category.preview!.slice(0, PREVIEW_LIMIT) : null);
